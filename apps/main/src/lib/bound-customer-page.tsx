...
/** Bind a booking project page that expects `{ customerId }`. */
export function createCustomerScopedPage(pageId: string, customerId: string) {
  ...
  const LazyPage = lazy(async () => {
    ...
    const Page = mod.default as ComponentType<{ customerId: string }>
    return {
      default: function Bound() {
        return <Page customerId={customerId} />
      },
    }
  })
